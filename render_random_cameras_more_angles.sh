...
EOL
  ...
  low_sample_command="$base_command $pbrt_file --outfile ${rendered_output_directory}/random_camera_${i}_low.exr --spp $spp_low --seed 123 --log-level verbose"
  $low_sample_command

  # Render the high-sample image
  high_sample_command="$base_command $pbrt_file --outfile ${rendered_output_directory}/random_camera_${i}_high.exr --spp $spp_high --seed 123 --log-level verbose"
  $high_sample_command
done